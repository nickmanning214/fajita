...
describe("Fajita.View", function() {
    ...
    describe("el", function() {
        ...
        describe("from parent (or regular view)", function() {
            ...
            it("Should display a subview", function() {
                var SubView = BaseView.extend({
                    defaults: {},
                    templateString: `How's it going?`
                });
                var ViewWithSubView = BaseView.extend({
                    defaults: {},
                    subViewImports: {
                    ...
                    templateString: `
                        <div class='wrapper'>{{subView}}</div>
                    `
                });

                var view = new ViewWithSubView;
                expect(view.el.querySelector(".wrapper").innerHTML).to.equal("How's it going?");
            });

            it("Should display a mapping", function() {
                ...
                var ViewWithSubView = BaseView.extend({
                    ...
                    templateString: `
                        <div class='wrapper'>{{subView:collection}}</div>
                    `
                });

                var view = new ViewWithSubView;
                window.v = view;
                expect(view.el.querySelectorAll("p").length).to.equal(5);
            });

            it("Should display a mapping (model)", function() {
